...
class AppleFrameworkIT : KotlinAndroid36GradleIT() {
    ...
    fun `check there aren't Xcode tasks without Xcode environment`() {
        with(Project("sharedAppleFramework")) {
            build("tasks") {
                assertSuccessful()
                assertNotContains("assembleAppleFramework")
                assertNotContains("embedAndSignAppleFramework")
            }
        }
    }
    ...
    fun `check all registered tasks with Xcode environment for Debug IosArm64 configuration`() {
        with(Project("sharedAppleFramework")) {
            ...
            build("tasks", options = options) {
                assertSuccessful()
                assertContains("assembleAppleFramework")
                assertContains("assembleSharedDebugAppleFrameworkIosArm64")
                assertContains("embedAndSignAppleFramework")

                assertNotContains("assembleSharedDebugAppleFrameworkIosX64")
            }
        }
    }
